...
#
# # sklearn packages
# from sklearn.metrics import roc_curve, auc, confusion_matrix, mutual_info_score, \
#     f1_score, jaccard_similarity_score, roc_auc_score, accuracy_score, precision_recall_curve
# from sklearn.metrics import mean_absolute_error, precision_score, recall_score
# from sklearn.gaussian_process import GaussianProcessRegressor
#
from utils import *
from utils.utils import *
# # from utils.hyper_opt import *
# # from utils.utils import *
...
info = dict()
info["dateTime"] = datetime.now().strftime("%Y%m%dT%H%M")
load_folder = r"./data/raw/"
######################################################
#                     LOAD DATA                      #
######################################################
# external information (demographic and weather)
census = pd.read_pickle(load_folder + "census_minmax_normed.pkl")  # some tracts are missing!! some that aren"t in the
weather = pd.read_pickle(load_folder + "weather_minmax_normed.pkl")

# physical coords and their tracts
valid_points = np.load(load_folder + "valid_points.npy")
valid_tracts = np.load(load_folder + "valid_tracts.npy")
invalid_tracts = np.load(load_folder + "invalid_tracts.npy")
tract2points = pd.read_pickle(load_folder + "tract2points.pkl")
point2tract = pd.read_pickle(load_folder + "point2tract.pkl")
tract_boundries = pd.read_pickle(load_folder + "tract_boundaries.pkl")
tract2km2 = pd.read_pickle(load_folder + "tract2km2.pkl")

# link between points and demographic data
point2spatial_info = pd.read_pickle(load_folder + "point2spatial_info.pkl")

# filter crimes by valid points
crimes = pd.read_pickle(load_folder + "crimes.pkl")

######################################################
#                PREPROCESS DATA                     #
######################################################
# DISCRITIZE IN TIME
# temporal coordniates for weather data
dT = "24H"
info["dT"] = dT
# DISCRITIZE IN SPACE
scale = 1 * np.array([10, 8])  # must be integer so that we can easily sample demographic data
dx, dy = scale * np.array([0.001, 0.001])
info["dx"] = float(dx)
info["dy"] = float(dy)
...
t_range = pd.date_range("2013-01-01", "2015-01-01", freq=dT)
t_range = t_range[:-11]
# crimes.DateTime = crimes.DateTime.dt.floor(dT) # DON"T ROUND, ONLY FLOOR, OTHERWISE THE WAETHER DATA DOESNT LINE UP PROPERLY.

tmin = pd.Series(crimes.DateTime.min()).astype(np.int64)[0]
...
# x_range = np.arange(crimes.X.min(),crimes.X.max()+dx,dx) # .001 because that is our smallest element dx can be bigger
...
demog_grid = np.zeros((37, y_size, x_size))  # grids with demographic info
...
# knn to fill in featureless cell with the values of the closest featureful cell
# SUGGESTION! replace knn with GMM to get a better average of the cells.
point2feats_res18 = pd.read_pickle(load_folder + "point2feats_res18.pkl")
...
# A = crimes[["t","b","TOTAL","THEFT", "BATTERY", "NARCOTICS","Arrest"]].values

# def make_grid__2(A,t_size,x_size,y_size):
...
#                            SAVE DATA                                  #
#########################################################################
save_folder = "./data/processed/T{}-X{}M-Y{}M/".format(dT, int(info["x in metres"]), int(info["y in metres"]))
...
plt.savefig(save_folder + "scatter_map.png")

plt.figure()
plt.title("Crimes")
plt.imshow(grids.max(0), cmap="viridis")
plt.savefig(save_folder + "crimes_max.png")
plt.figure()
plt.title("Demographics")
plt.imshow(demog_grid.max(0), cmap="viridis")
plt.savefig(save_folder + "demographics_max.png")
plt.figure()
plt.title("Street View Info")
plt.imshow(street_grid.max(0), cmap="viridis")
plt.savefig(save_folder + "street_grid_max.png")
...
# np.savez(folder+"weather_vectors.npz", weather_vectors) # way too short should be more years
