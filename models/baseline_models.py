...
from utils.time_series import get_max_steps, periodic_average


class PeriodicAverage:
    """
    Running average model with a periodic time step between samples of the running average.
    Is useful for scenarios when the signals are time dependent on features like the time of day or day of week.
    """
    ...
    def fit(self, data):
        ...
        self.fitted = True
        print(f"fitted periodic average: step ({self.step}) and max_steps ({self.max_steps})")

    def transform(self, data):
        if not self.fitted:
            raise RuntimeError(f"Model needs to be fitted to the data first: model.fitted = {self.fitted}")

        return periodic_average(data, self.step, self.max_steps)
...
if __name__ == "__main__":
    ...
    # use grids as we get average for that time
    grids_pa = get_periodic_average(data=grids[:len(grids)], step=period)
    grids_pama = grids_pa * grids_ma

    grids_pa = np.nan_to_num(grids_pa)
    grids_ma = np.nan_to_num(grids_ma)
    grids_pama = np.nan_to_num(grids_pama)
    ...
    grids_pa = grids_pa[window - 1:]
    grids_pama = grids_pama[window - 1:]
