...
def save_checkpoint(tag, model, optimiser, conf,
                    ...
                    trn_batch_losses):
    """
    save_checkpoint saves loss, model and optimizer information under the model directory.
    Is useful during training loops.
    Model, optimiser are saved separately in .pth files.
    Losses are all saves together in single .npz file.
    
    :param tag: targ used to annotate the checkpoint type, e.g. train, test, val, final.
    :param model: PyTorch model with trained weights
    :param optimiser: PyTorch optimizer linked to model weights
    :param conf: config object with settings for the model/optimiser
    :param val_batch_losses: list of validation set losses for each batch so far
    :param val_epoch_losses: list of validation set losses for each epoch so far
    :param trn_epoch_losses: list of training set losses for each epoch so far
    :param trn_batch_losses: list of training set losses for each batch so far
    :return: 
    """
...
def train_model_final(
    ...
    """
    train model using the train_validation_set until the specified conf.max_epochs: no early stopping is used
    this function is usually used after the validation set has been used to determine the mac number of epochs, i.e.
    when the model starts overwriting. By including the the training and validation set the model is more likely to
    generalise better given the same amount of epochs.

    :param model: torch model
    :param optimiser: torch optimizer
    :param loaders: batch loader class containing train, validation and test batch loaders
    :param train_epoch_fn: function used to train model for a single epoch
    :param loss_fn: loss function used in training process, can be CrossEntropy, MSE or MAE depending on model
    :param conf: configure object containing all preferences for model, optimisations and training loops
    ...
    Notes
    -----
    train_model_final will log all information to model folder as well as produce training and validation loss curve plots
    """
    ...
    # Training loop
    model.train()  # only set model to training mode once as we don't evaluate the model during the evaluation loops
